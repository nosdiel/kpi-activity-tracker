...
// -----------------------------------------------------------------------------
// CATERING SALES (by dining option / source) — feeds QTR Report Catering column
// -----------------------------------------------------------------------------

type CateringDay = { location_id: string; business_date: string; amount: number };

async function toastCateringDay(
  base: string,
  accessToken: string,
  restaurantGuid: string,
  businessDate: string,
  cateringGuids: Set<string>
): Promise<number> {
...
async function toastCateringDiningOptionGuids(
  base: string,
  accessToken: string,
  restaurantGuid: string
): Promise<Set<string>> {
...
async function squareCateringDay(
  accessToken: string,
  locationId: string,
  businessDate: string
): Promise<number> {
  ...
  do {
    ...
    const json = (await res.json()) as {
      orders?: Array<{
        source?: { name?: string };
        line_items?: Array<{ name?: string; catalog_object_id?: string; variation_name?: string; category_name?: string }>;
    ...
    for (const loc of (locs as any[]) ?? []) {
      try {
        if (loc.pos_provider === "square" && loc.square_location_id && loc.square_access_token) {
          ...
          const base = (loc.toast_api_url || TOAST_BASE).replace(/\/+$/, "");
          const creds = pickToastCreds(loc, "standard");
          const token = await toastAccessTokenWithBase(base, creds.clientId, creds.clientSecret);
